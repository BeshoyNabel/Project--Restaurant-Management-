...
$(document).ready(function () {
  ...
  $(".next").click(function () {
    ...
    if (current_fs.find("h2").text().trim() === "Customer Information") {
      if (
        current_fs.find('input[name="name"]').val() === "" ||
        current_fs.find('input[name="phone"]').val() === ""
      ) {
        alert("Please fill in all required fields.");
        return;
      } else {
        customerName = current_fs.find('input[name="name"]').val().trim();
        customerPhone = current_fs.find('input[name="phone"]').val().trim();
    ...
    } else if (current_fs.find("#order_summary")) {
